...
func assertCntIsAvailable(c *check.C, cname, sname, name string) {
...
func assertCntNotAvailable(c *check.C, cname, sname, name string) {
	if isCntPresent(c, cname, sname, name) {
...
func (s *DockerSuite) TestDockerPublishServiceFlag(c *check.C) {
	...
	// Detach the backend
	runCmd = exec.Command(dockerBinary, "service", "detach", cid, "telnet.production")
	out, _, err = runCommandWithOutput(runCmd)
	c.Assert(err, check.IsNil)
