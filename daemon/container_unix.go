...
func (container *Container) buildSandboxOptions(ctx context.Context, n libnetwork.Network) ([]libnetwork.SandboxOption, error) {
	var (
...
func (container *Container) updateNetworkSettings(n libnetwork.Network) error {
	...
	for _, s := range settings.Networks {
		sn, err := findNetwork(controller, s)
		if err != nil {
			continue
		}
...
func (container *Container) updateNetwork(ctx context.Context) error {
	...
	options, err := container.buildSandboxOptions(ctx, n)
	if err != nil {
...
func (container *Container) allocateNetwork(ctx context.Context) error {
	...
	for _, n := range settings {
		if err := container.connectToNetwork(ctx, n, updateSettings); err != nil {
			if updateSettings {
				return err
			}
...
func (container *Container) ConnectToNetwork(ctx context.Context, idOrName string) error {
	...
	return container.connectToNetwork(ctx, idOrName, true)
}

func (container *Container) connectToNetwork(ctx context.Context, idOrName string, updateSettings bool) error {
	...
	n, err := findNetwork(controller, idOrName)
	if err != nil {
		return err
	}
...
// Should ideally be a libnetwork convenience function
func findNetwork(controller libnetwork.NetworkController, id string) (libnetwork.Network, error) {
	// Find by Name
	n, err := controller.NetworkByName(id)
	...
	if len(list) > 1 {
		return nil, libnetwork.ErrInvalidID(id)
	}
...
func (container *Container) releaseNetwork() {
	...
	for _, ns := range networks {
		n, err := findNetwork(controller, ns)
		if err != nil {
			continue
		}
