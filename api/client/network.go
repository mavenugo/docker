...
func consolidateIpam(subnets, ranges, gateways, auxaddrs []string) ([]network.IpamData, error) {
	if len(subnets) < len(ranges) || len(subnets) < len(gateways) || len(subnets) < len(auxaddrs) {
		return nil, fmt.Errorf("every ip-range or gateway or auxilary-address must have a corresponding subnet")
	}
	iData := map[string]*network.IpamData{}

	// Populate subnets into consolidation map
	for _, s := range subnets {
		if _, ok := iData[s]; !ok {
			iData[s] = &network.IpamData{Subnet: s, AuxAddress: map[string]string{}}
		} else {
			return nil, fmt.Errorf("cannot configure multiple subnets with the same value %s", s)
		}
	}
	...
	for _, aa := range auxaddrs {
		match := false
		for _, s := range subnets {
			a := strings.Split(aa, "=")
			if len(a) != 2 {
				return nil, fmt.Errorf("aux-address must be defined in the form Key=Value")
			}

			ok, err := subnetMatches(s, a[1])
			if err != nil {
				return nil, err
			}
			if !ok {
